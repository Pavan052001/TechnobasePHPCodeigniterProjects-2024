...
class LeadSource_model extends CI_Model
{
    ...
    public function ExportData($table,$fields='',$condition='')
	{
		...
		{
			$return[] = $this->GetData("lead_sources","source_title,description,status,created","id='".$id[$j]."'","","","","1");
		}
    ...
    public function ImportData($record)
	{
		...
		{	
			$this->db->select('*');
			$this->db->where('source_title',$record[0]);
			...
			if(count($response) == 0)
			{
				$import = array(
					...
					'source_title' => ucwords(trim($record[0])),
                    'description'=>trim($record[1]),
					'created' => date("Y-m-d H:i:s"),
				);
				$this->db->insert('lead_sources',$import);
			}
		}
	} 
}
