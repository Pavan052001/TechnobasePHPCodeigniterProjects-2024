<?php
class LeadStage_model extends CI_Model
{
    ...
    public function ExportData($table,$fields='',$condition='')
	{
		...
		{
			$return[] = $this->GetData("lead_stage","s_title,description,status,created","id='".$id[$j]."'","","","","1");
		}
		...
		$filename ="LeadStagexport_".$date.".csv";
    ...
    public function ImportData($record)
	{
		...
		{	
			$this->db->select('*');
			$this->db->where('s_title',$record[0]);
			$return = $this->db->get('lead_stage');
			...
			if(count($response) == 0)
			{
				$import = array(
					...
					's_title' => ucwords(trim($record[0])),
                    'description'=>trim($record[1]),
					'created' => date("Y-m-d H:i:s"),
				);
				$this->db->insert('lead_stage',$import);
			}
		}
	} 
}
