    ...
    <section class="section">
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="card">
            ...
            <div class="card-body">
                ...
                <form method="post" action="<?= site_url("CountryController/export")?>">
									&nbsp;<button type="submit" id="countryidbtn" class="btn btn-primary mt-3">Export</button>
									<input type="hidden" class="countrycollectid " name="countrycollectid"/>
								</form>


              <a href="<?php echo site_url('CountryController/create') ?>" class="btn btn-primary bx-pull-right me-5">create
              </a>
              <br>
              <br>

              <!-- Table with stripped rows -->
             <form method="post" action="<?php echo site_url("CountryController/deleteall_action")?>">
                ...
                <tbody>
                  <?php if (!empty($country)): ?>
                    ...
                    foreach ($country as $row): ?>
                      <tr>
                        <td><input type="checkbox" class="countryid" name="selector[]" value="<?php echo $row->id ?>"></td>
                     <td><?php echo $s ;?></td>
                        <td><?php echo $row->country_name; ?></td>

                        <td><img src="<?= base_url();?>uploads/flag/<?= $row->country_flag?>" width="50px"/></td> 
                        ...
                        <td>
                          ...
                          <a href="<?php echo site_url('CountryController/updateCountry/' . $row->id) ?>" class=" me-3"><i
                              class="bi bi-pencil"></i></a>
                          <a href="<?php echo site_url('CountryController/delete/' . $row->id) ?>"  onclick="return confirm('Do you really want to delete this record')"
        ...
        <div class="card col-lg-6 col-md-6 col-sm-6 col-xs-12 box">
								<form method="post" action="<?php echo site_url("CountryController/import") ?>" enctype="multipart/form-data">
...
<script>
			$("#countryidbtn").click(function(){
				var ID = new Array();
				 $('.countryid').each(function(){
					 ID.push($(this).val());
				 });
				 $('.countrycollectid').val(ID);
			});
		</script>
...
$(document).ready(function() {
    $('.btn-status').click(function() {
        ...
        $.ajax({
            url: "<?php echo site_url('CountryController/updateStatus'); ?>",
            type: "POST",
             data: { id:Id },
            dataType: "json",
